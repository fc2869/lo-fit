...
HF_NAMES = {
    ...
    'llama2_chat_7B': '/data/shared_resources/models/llama2/hf/NousResearch-Llama-2-7b-chat-hf', 
    # 'llama2_chat_7B':'likenneth/honest_llama2_chat_7B',
    'honest_llama2_chat_7B': 'likenneth/honest_llama2_chat_7B', 
...
def main(): 
    ...
    feature_dir = '/data/users/fcyin/iti_features/'
    head_wise_activations = np.load(f"{feature_dir}{args.model_name}_{args.dataset_name}_head_wise.npy")
    labels = np.load(f"{feature_dir}{args.model_name}_{args.dataset_name}_labels.npy")
    head_wise_activations = rearrange(head_wise_activations, 'b l (h d) -> b l h d', h = num_heads)

    # tuning dataset: no labels used, just to get std of activations along the direction
    activations_dataset = args.dataset_name if args.activations_dataset is None else args.activations_dataset
    tuning_activations = np.load(f"{feature_dir}{args.model_name}_{activations_dataset}_head_wise.npy")
    tuning_activations = rearrange(tuning_activations, 'b l (h d) -> b l h d', h = num_heads)
    tuning_labels = np.load(f"{feature_dir}{args.model_name}_{activations_dataset}_labels.npy")
    ...
    for i in range(args.num_fold):
        ...
        # train_set_idxs = np.random.choice(train_idxs, size=40, replace=False)
        ...
        print(len(probes),len(top_heads))
        ...
        def lt_modulated_vector_add(head_output, layer_name, start_edit_location='lt'): 
            ...
            for head, direction, proj_val_std in interventions[layer_name]:
                ...
                if start_edit_location == 'lt': 
                    ...
                    # head_output[:, -1, head, :] = args.alpha * proj_val_std * direction_to_add
                    # head_output[:, -1, head, :] = torch.zeros_like(head_output[:, -1, head, :]).to(head_output[:, -1, head, :].get_device())
                    
                else: 
                    head_output[:, start_edit_location:, head, :] += args.alpha * proj_val_std * direction_to_add
                    # head_output[:, start_edit_location:, head, :] = args.alpha * proj_val_std * direction_to_add
                    # head_output[:, start_edit_location:, head, :] = torch.zeros_like(head_output[:, start_edit_location:, head, :]).to(head_output[:, start_edit_location:, head, :].get_device())
