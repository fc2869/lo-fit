...
def evaluate_mquake(eval_dataset,model_name,model,tokenizer,fname,batch_size=16,max_new_token=16,apply_chat_template=False):
    ...
    with torch.no_grad():
        for i in iterator:
            ...
            outputs = model.generate(**inputs_b,max_new_tokens=max_new_token,do_sample=False)
    ...
    return generated
def evaluate_clutrr(eval_dataset,model_name,model,tokenizer,fname,batch_size=16,max_new_token=16,apply_chat_template=False):
    results_dir = os.path.join(fname,'outputs.json')
    result_json = []
    tokenizer.padding_side = 'left'
 
    inputs = eval_dataset[key]['prompt']
    iterator = range(0, len(inputs), batch_size)
    generated = []
    with torch.no_grad():
        for i in iterator:
    ...
    for i in range(len(generated)):
        ...
        gold_label = eval_dataset[key]['target_text'][i].lower()
        entry_corr= 0
        if ans == gold_label:
            entry_corr = 1
        corr+=entry_corr
    ...
    print('Test Set:',key,'Accuracy:',corr/len(generated))
    json.dump(result_json,open(results_dir,'w'))
    return generated
def evaluate_tqa(eval_dataset,fname,model,tokenizer,metrics=metrics,model_name=None,verbose=False):
    mnames = {
    ...
    curr_fold_results = alt_tqa_evaluate(
        {mnames[model_name]: self.model}, 
        eval_dataset['data_dir'], 
        f'../tqa_results/answer_dump/{fname}.csv',
        f'../tqa_results/summary_dump/{fname}.csv',
        metrics=metrics,
        device="cuda", 
        tokenizer=tokenizer,
